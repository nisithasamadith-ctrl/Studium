...
> **Database**: I will set up **Prisma** with PostgreSQL. You will need a running PostgreSQL instance and provide the `DATABASE_URL` in a `.env` file later.
...
- Dependencies: `express`, `cors`, `dotenv`, `prisma`, `@prisma/client`, `openai`, `firebase-admin` (or similar for auth verification).
...
- Database schema definition:
    ...
    - `Task`: id, title, course, deadline, priority, isCompleted, userId
    - `Assignment`: id, title, course, dueDate, difficulty, userId
    - `FocusSession`: id, duration, timestamp, userId
    - `Forest`: (derived from FocusSessions or explicit table)
